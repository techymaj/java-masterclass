...
//        try {
//            // an older way to register the driver
...
//            connection.setAutoCommit(false);
    ...
    private static boolean insertRecord(Statement statement, String table,
        ...
        String colValues = String.join("','", columnValues); // prev and next each get '
    ...
    private static boolean updateRecord(Statement statement, String table, String matchedColumn, String matchedValue,
                                        String updatedName, String updatedValue)
            throws SQLException {

        String query = "UPDATE %s SET %s = '%s' WHERE %s='%s'"
                .formatted(table, updatedName, updatedValue, matchedColumn, matchedValue);
        System.out.println(query);
        statement.execute(query);
        int recordsUpdated = statement.getUpdateCount();
        if (recordsUpdated > 0) {
            executeSelect(statement, table, updatedName, updatedValue);
        }
